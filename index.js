...
function createSymbolWithDescription(property) {
  ...
  return symbol.toString().slice(7, -1); //Повертаємо опис символа symbol
}
...
function createAndCompareTwoSymbols(value) {
  ...
  return symbol1 === symbol2; // Створюємо два символи з однаковим описом value, та повертаємо результат порівняння двох символів
}
...
function useSymbolToStringTag() {
  ...
  myObject[Symbol.toStringTag] = "CustomObject"; // Встановлюємо властивість Symbol об'єкту myObject на значення "CustomObject".
  return myObject[Symbol.toStringTag]; // Повертаємо рядкове представлення myObject[Symbol].
}
...
let rangeObject = {
  ...
  [Symbol.iterator]() {
    ...
    let current = this.from; // this.current присвоюємо this.from
    const last = this.to;
    return {
      // Повертаємо this
      next() {
        ...
        if (current <= last) {
          // Використовуйте if
          return { value: current++, done: false }; // Якщо current менше to, повертаємо об'єкт з властивістю "value",що містить поточне значення ,
...
function useSymbolIterator(obj) {
  ...
  const result = []; // Додаємо кожне значення до масиву "result"
...
let myObject = {
  ...
  [Symbol.iterator]() {
    let currentIndex = 0; // Оголошення методу Symbol.iterator для об'єкта "myObject"
    return {
      //Повертаємо this
      ...
      next: () => {
        ...
        if (currentIndex < this.category.length) {
          ...
          const value = this.category[currentIndex].name; //Створюємо змінну value якій присвоємо властивість name елемента масиву category з індексом currentIndex
          currentIndex++; // Збільшимо currentIndex на одиницю
